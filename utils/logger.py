...
#    file_name = f"..\\logs\\log_" + str(datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")) + ".log"
    ...
    @classmethod  # этот метод (write_log_to_file) может обращаться к переменным этого класса (Logger)
    def write_log_to_file(cls, data: str):  # при '@classmethod' указывать не 'self', a 'cls'
        """   Метод при помощи которого открываем файл для записи данных   """
    ...
    def add_response(cls, result: Response):
        """   Метод для получения данных по ответу   """
